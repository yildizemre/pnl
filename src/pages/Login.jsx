  ...
  return (
    <div className="login-shell relative min-h-screen overflow-hidden bg-[var(--bg-page)]">
      ...
      <div className="absolute right-5 top-5 z-20 flex items-center gap-2">
        <div className="flex items-center gap-0.5 rounded-lg border border-[var(--border)] bg-[var(--bg-card)]/80 p-0.5 backdrop-blur-md">
          {["TR", "EN"].map((l) => (
            <button
              key={l}
              type="button"
        ...
        <button
          type="button"
          onClick={toggle}
          className="flex h-10 w-10 items-center justify-center rounded-full border border-[var(--border)] bg-[var(--bg-card)]/80 text-[var(--text-muted)] backdrop-blur-md transition hover:text-[var(--text-primary)]"
          aria-label={t.tema}
        >
      ...
      <div className="relative z-10 grid min-h-screen lg:grid-cols-[1.05fr_1fr]">
        <div className="login-brand-panel relative hidden overflow-hidden border-r border-[var(--border)] lg:flex">
            ...
            <div className="flex w-full max-w-md flex-col items-center text-center space-y-6">
              ...
              <div className="flex flex-wrap justify-center gap-2">
                {tags.map((tag) => (
                  <span
                    key={tag}
                    className="rounded-full border border-cyan-500/25 bg-cyan-500/10 px-3 py-1 text-xs font-medium text-cyan-500"
                  >
                    {tag}
                  </span>
                ))}
        ...
        <div className="flex items-center justify-center px-6 py-16 sm:px-12">
          <div className="w-full max-w-[400px]">
            ...
            <div className="login-card rounded-3xl border border-[var(--border)] p-8 sm:p-10">
              <div className="mb-8">
                <p className="text-xs font-semibold uppercase tracking-[0.2em] text-cyan-500">{t.hosgeldiniz}</p>
                <h2 className="mt-2 text-2xl font-bold text-[var(--text-primary)]">{t.hesabaGiris}</h2>
