...
export default function HeaderBar({ title, subtitle, onMenuClick, onNavigate, unread, liveSummary }) {
  ...
  return (
    <header className="sticky top-0 z-20 border-b border-[var(--border)] bg-[var(--bg-header)]/90 px-4 py-2.5 backdrop-blur-xl sm:px-6 lg:px-8">
      <div className="flex items-center justify-between gap-3">
        ...
        <div className="flex max-w-[55%] shrink-0 flex-wrap items-center justify-end gap-1 sm:max-w-none sm:gap-2">
          <div className="hidden items-center gap-0.5 rounded-lg border border-[var(--border)] bg-[var(--bg-card)]/60 p-0.5 md:flex">
            {["TR", "EN"].map((l) => (
              <button
                key={l}
                type="button"
          ...
          >
            <span className={`h-1.5 w-1.5 rounded-full ${aiAktif ? "bg-emerald-500 animate-pulse" : "bg-amber-500"}`} />
            {aiAktif ? "AI" : "…"}
          </span>
          ...
          <div className="relative" ref={ref}>
            <button
              ...
              className="relative rounded-xl border border-[var(--border)] bg-[var(--bg-card)]/80 p-2 text-[var(--text-muted)] hover:border-cyan-500/30 hover:text-cyan-500"
            >
              <Bell className="h-[18px] w-[18px]" />
              {displayUnread > 0 && (
                <span className="absolute -right-1 -top-1 flex h-4 min-w-[16px] items-center justify-center rounded-full bg-gradient-to-r from-red-500 to-orange-500 px-1 text-[10px] font-bold text-white">
            ...
            {open && (
              <div className="absolute right-0 z-50 mt-2 w-[min(100vw-2rem,380px)] overflow-hidden rounded-2xl border border-[var(--border)] bg-[var(--bg-card)] shadow-2xl">
                <div className="border-b border-[var(--border)] bg-gradient-to-r from-cyan-500/10 to-violet-500/5 px-4 py-3">
                ...
                <div className="max-h-72 overflow-y-auto">
                  {recent.length === 0 ? (
                    ...
                    recent.map((n) => (
                      <div key={n.id} className="flex gap-3 border-b border-[var(--border)] px-4 py-3 hover:bg-[var(--bg-hover)]">
                        <div className="h-10 w-12 shrink-0 overflow-hidden rounded-lg border border-[var(--border)] bg-[var(--bg-page)]">
                          {n.gorsel ? (
                            ...
                            <div className="flex h-full items-center justify-center text-[10px] text-cyan-500/50">AI</div>
                          )}
                        </div>
                ...
                <button type="button" onClick={() => { setOpen(false); onNavigate?.("bildirimler"); }} className="w-full py-2.5 text-xs font-semibold text-cyan-500 hover:bg-cyan-500/10">
                  {t.tumunuGor}
                </button>
              </div>
            )}
