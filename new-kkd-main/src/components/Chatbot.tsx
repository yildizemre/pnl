...
export default function Chatbot() {
  ...
  const [messages, setMessages] = useState<Message[]>([
    { role: 'bot', text: 'Merhaba! Ben HypeVision AI Asistan. Kamera, ihlal ve güvenlik verileri hakkında sorularınızı yanıtlayabilirim.', time: now() },
  ]);
  ...
  const sendMessage = async (text: string) => {
    ...
    setLoading(true);
    try {
      const res = await fetchChatResponse(trimmed);
      const botMsg: Message = { role: 'bot', text: res.response, time: now() };
      setMessages((p) => [...p, botMsg]);
    } catch {
      setMessages((p) => [...p, { role: 'bot', text: 'Üzgünüm, şu anda yanıt veremiyorum. Lütfen tekrar deneyin.', time: now() }]);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <>
      ...
      >
        <div className="bg-white rounded-3xl shadow-[0_16px_60px_rgba(11,60,93,0.22)] border border-slate-200/80 flex flex-col overflow-hidden" style={{ maxHeight: '620px', height: '620px' }}>
          {/* Header */}
          <div className="px-4 py-3.5 flex items-center justify-between flex-shrink-0"
            ...
            <div className="flex items-center gap-2.5">
              ...
              <div>
                ...
                <p className="text-white/50 text-[10px]">Çevrimiçi · Hemen yanıt veriyor</p>
              </div>
            </div>
          ...
          {/* Messages */}
          <div ref={scrollRef} className="flex-1 overflow-y-auto px-3 py-4 space-y-3 bg-slate-50/50">
            {messages.map((msg, i) => (
              <div key={i} className={`flex items-end gap-2 ${msg.role === 'user' ? 'flex-row-reverse' : 'flex-row'}`}>
                ...
                <div className={`flex flex-col max-w-[78%] ${msg.role === 'user' ? 'items-end' : 'items-start'}`}>
                  ...
                  <p className="text-[9px] text-slate-400 mt-1 px-1">{msg.time}</p>
                </div>
              </div>
            ))}
            ...
            {loading && (
              <div className="flex items-end gap-2">
                ...
                <div className="bg-white rounded-2xl rounded-tl-sm px-4 py-2.5 flex items-center gap-1.5 border border-slate-100">
                  {[0, 1, 2].map((i) => (
          ...
          {messages.length <= 2 && !loading && (
            <div className="px-3 pb-2 flex-shrink-0">
              <p className="text-[9px] text-slate-400 mb-1.5 font-bold uppercase tracking-widest flex items-center gap-1">
                <Sparkles size={9} /> Hızlı Sorular
              </p>
              <div className="flex flex-wrap gap-1">
                {QUICK_QUESTIONS.map((q) => (
                  <button key={q} onClick={() => sendMessage(q)}
                    className="text-[10px] font-semibold px-2 py-1 rounded-full bg-[#00BCD4]/8 text-[#0B3C5D] hover:bg-[#00BCD4]/15 transition-colors border border-[#00BCD4]/15">
                    {q}
                  </button>
                ))}
              </div>
          ...
          {/* Input */}
          <div className="px-3 pb-3 pt-1.5 border-t border-slate-100 flex-shrink-0">
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 bg-slate-100 rounded-xl px-3 py-2 text-xs text-slate-700 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-[#00BCD4]/40 transition"
              />
